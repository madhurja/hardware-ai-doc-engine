...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--type", choices=DOCUMENT_TYPES, default="user_manual", help="Document package type.")
...
def main() -> int:
    ...
    markdown = generator.generate_document(args.type, hardware_profile)

    print_progress(3, "Compiling enterprise document")
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    title = args.type.replace("_", " ").title()
    output_dir = Path(args.output_dir)
    exporter = PDFExporter()
    if args.html_only:
        output_path = exporter.export_html(title, markdown, output_dir / f"{args.type}_{timestamp}.html")
    else:
        output_path = exporter.export_pdf(title, markdown, output_dir / f"{args.type}_{timestamp}.pdf")

    print_progress(4, "Recording local portfolio snapshot")
    if not args.skip_git:
        git_snapshot()

    print(f"\nDone. Document created: {output_path}")
    if not args.skip_git:
